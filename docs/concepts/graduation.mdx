...
Since `curve_virtual_sol / curve_virtual_tokens` is the price where the curve ended, the pool's opening spot equals that price, and graduation itself causes no price gap. Under the launch convention (curve opens at a quarter of the index and spans 4x over the raise; see [Launch curve](/concepts/launch-curve)), the closing price is the index itself, so the AMM opens trading at the collectible's live price. The seed stays in the market's pool token account, a program-controlled account no person can sign for.
